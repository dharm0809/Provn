...
**Stage D:** scrape the WAL (anonymised) for ~50,000 unlabelled responses → contrastive pretraining corpus.
...
#### Task 9.1: ADWIN drift detector

`flywheel/adwin_detector.py` — runs in a background asyncio task on the gateway. Tracks per-field-type confidence over time. When ADWIN trips on a per-canonical-label confidence drop, emits a `drift_detected` event.
