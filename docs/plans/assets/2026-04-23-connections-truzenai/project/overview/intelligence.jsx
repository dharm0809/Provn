/* Walacor Gateway — Intelligence View
   Sub-tabs: Production · Candidates · Promotion History · Verdict Inspector */
...
// ── Utility: mini sparkline (accuracy trend) ───────────────────────
...
// ── Header metric bar (top-of-view: models, candidates, pending shadow) ──
function IntelHeader({ models, candidates }) {
  ...
  const avgAcc = models.reduce((s, m) => s + m.accuracy, 0) / models.length;
          ...
          <div key={m.model_name} className="card prod-card">
            ...
            <div className="prod-hero">
              <div className="prod-hero-acc">
                <div className="prod-hero-val gold">{(m.accuracy * 100).toFixed(1)}<span style={{fontSize:14, color:'var(--text-muted)'}}>%</span></div>
            ...
            <dl className="prod-dl">
              ...
              <div><dt>Predictions (24h)</dt><dd className="mono">{formatNumber(m.predictions_24h)}</dd></div>
              <div><dt>Predictions (7d)</dt><dd className="mono">{formatNumber(m.predictions_7d)}</dd></div>
              <div><dt>Trailing accuracy</dt><dd className="mono">{(m.trailing_accuracy * 100).toFixed(2)}%</dd></div>
            </dl>
            ...
            <div className="prod-foot">
              <div className="prod-foot-left">
                ...
                <div className="prod-foot-v">
                  ...
                  <span className="prod-approver">by {m.last_promotion.approver}</span>
                </div>
              </div>
      ...
      {candidates.map(c => {
        ...
        return (
          <div key={`${c.model_name}:${c.version}`}
            ...
            {/* Progress toward shadow completion */}
            {noShadow && (
              ...
              <>
                ...
                <div className="cand-bars">
                  <div className="cand-bar-row">
                    ...
                    <div className="cand-bar-track"><div className="cand-bar-fill prod" style={{ width: `${m.production_accuracy * 100}%` }} /></div>
                    <span className="cand-bar-val mono">{fmtPct(m.production_accuracy, 2)}</span>
                  </div>
                  <div className="cand-bar-row">
                    <span className="cand-bar-lbl">cand</span>
                    <div className="cand-bar-track"><div className="cand-bar-fill cand" style={{ width: `${m.candidate_accuracy * 100}%` }} /></div>
...
function PromoteModal({ candidate, onClose, onConfirm }) {
  ...
  const confirm = () => {
    setBusy(true);
    setTimeout(() => { onConfirm(); setBusy(false); }, 900);
  };
...
function RejectModal({ candidate, onClose, onConfirm }) {
  ...
  const confirm = () => {
    setBusy(true);
    setTimeout(() => { onConfirm(reason.trim() || 'manual_rejection'); setBusy(false); }, 700);
  };
...
function RollbackModal({ model, onClose, onConfirm }) {
  ...
  const confirm = () => {
    setBusy(true);
    setTimeout(() => { onConfirm(); setBusy(false); }, 1100);
  };
...
function IntelligenceView({ showToast }) {
  const [sub, setSub] = React.useState('production');
  const [models, setModels] = React.useState(() => IntelData.models);
  const [candidates, setCandidates] = React.useState(() => IntelData.candidates);
  const [historyModel, setHistoryModel] = React.useState('intent');
  const [historyEvents, setHistoryEvents] = React.useState(() => IntelData.genHistory('intent'));

  const [verdictModel, setVerdictModel] = React.useState('intent');
  const [divergenceOnly, setDivergenceOnly] = React.useState(true);
  const [limit, setLimit] = React.useState(100);
  const [verdictData, setVerdictData] = React.useState(() => IntelData.genVerdicts('intent', true, 100));
  const [retrainStatus, setRetrainStatus] = React.useState('');
  ...
  React.useEffect(() => {
    setHistoryEvents(IntelData.genHistory(historyModel));
  }, [historyModel]);

  React.useEffect(() => {
    setVerdictData(IntelData.genVerdicts(verdictModel, divergenceOnly, limit));
  }, [verdictModel, divergenceOnly, limit]);

  const handlePromote = () => {
    const c = promoteTarget;
    setCandidates(prev => prev.filter(x => !(x.model_name === c.model_name && x.version === c.version)));
    setModels(prev => prev.map(m => m.model_name === c.model_name
      ? { ...m, active_version: c.version, generation: m.generation + 1,
          last_promotion: { candidate_version: c.version, approver: 'alex.chen@acme.io', timestamp: new Date().toISOString(), dataset_hash: c.dataset_hash } }
      : m));
    setPromoteTarget(null);
    showToast(`▲ Promoted ${c.model_name} → ${c.version.slice(0, 12)}…`);
  };
  ...
  const handleReject = (reason) => {
    ...
    showToast(`✕ Rejected ${c.model_name} (${reason})`);
  };

  const handleRollback = () => {
    setRollbackOpen(false);
    showToast(`↺ Rolled back ${historyModel}`);
    setHistoryEvents(IntelData.genHistory(historyModel));
  };
  ...
  return (
    <div className="intel-view">
      ...
      {sub === 'candidates' && <CandidatesView candidates={candidates}
                                               onPromote={setPromoteTarget}
                                               onReject={setRejectTarget} />}
      {sub === 'history' && <HistoryView model={historyModel} setModel={setHistoryModel}
