...
// import { getConnections } from '../api'; // wired once the backend ships
...
/* ── Mock scenarios (inline per spec; backend not yet live) ──────── */
...
export default function Connections({ navigate }) {
  ...
  const [scenario, setScenario] = useState('amber'); // 'happy' | 'amber' | 'red'
  const [snapshot, setSnapshot] = useState(() => mocks.amber);
  const [loading, setLoading] = useState(false);
  const [error, setError]     = useState(null);
  const [openTileId, setOpenTileId] = useState(null);
  const [nowTick, setNowTick] = useState(0); // forces "N ago" recomputation

  /* Load whenever scenario changes. Real wire-up will swap this for
     `getConnections()`. Kept inline to honour the "mock data inline,
     three scenarios" rule in the prompt. */
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    setError(null);
  ...
  /* 3s poll tick — real endpoint is 3s TTL, we mirror that cadence.
     In mock mode this just re-stamps "N seconds ago" labels. */
  useEffect(() => {
...
function Intro({ snapshot, scenario, setScenario, loading, error }) {
