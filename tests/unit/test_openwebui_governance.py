...
def _mock_ctx(
    ...
    if has_chain:
        chain = AsyncMock()
        chain.next_chain_values.return_value = (0, "0" * 128)
...
async def test_apply_session_chain():
    ...
    assert record["previous_record_hash"] == "0" * 128
    assert "record_hash" in record
    ctx.session_chain.update.assert_called_once()
...
async def test_session_chain_owui_prefix():
    ...
    ctx.session_chain.update.assert_called_once()
    call_args = ctx.session_chain.update.call_args[0]
    assert call_args[0] == "owui:my-chat-42"
