...
## Apr-24 session (branch consolidation + OpenWebUI integration)
- **Branches consolidated into main**: rebased `feature/phase25-onnx-self-learning` onto `origin/main` (234 commits replayed, 0 conflicts — the plugin-file restructure origin/main did in `31be5b6` auto-merged cleanly because phase25 never touched the deleted files or the new ones). Cherry-picked `e48fdfa` (sonam-plugin-openwebui → Ollama proxy) and `3a9e1b0` (sonam-plugin-rnd → plugin governance + model_hash). Fast-forwarded `main` to the result. Dead branches on origin (feature/phase25-onnx-self-learning, feature/data-integrity-engine, sonam-plugin-openwebui, sonam-plugin-rnd, feat/stream_combined) **cannot be force-pushed or deleted via CLI** — GitHub repo rule `GH013 Cannot delete this branch` blocks it. Prune via GitHub Settings → Rules or the Branches UI.
- **OpenWebUI as native Ollama client — `src/gateway/ollama_proxy.py`**: 4 handlers (`/api/tags`, `/api/ps`, `/api/version`, `/api/show`) proxy to `settings.provider_ollama_url`. Registered in `main.py`; `/api/` and `/v1/openwebui/` both added to `api_key_middleware` exempt set (`_plugin_paths`) and `completeness_middleware` exempt prefix tuple. Lets OpenWebUI register the gateway as an *Ollama* connection (not just OpenAI-compatible). Fail-open: 502 on Ollama unreachable, 503 if `ctx.http_client is None`.
- **Plugin event governance — `src/gateway/openwebui/governance.py`** (403 lines, 22 tests): `process_plugin_event(body)` reproduces the proxy-path governance pipeline for OpenWebUI plugin events that never went through the proxy. Outlet events run the full pipeline (attestation → pre-policy → content analysis → session chain → build_execution_record → dual-write via `ctx.storage`). Inlet events get the lightweight record. Session IDs are namespaced `owui:{chat_id}` to avoid colliding with proxy chains. Blocks surface as `blocked_post_facto` — audit-only, never actually blocks (the LLM already answered). Fail-open at every step. Wired into `/v1/openwebui/events` handler. Toggle: `WALACOR_PLUGIN_EVENT_GOVERNANCE_ENABLED=true` (default). **Ported from the old SHA3 Merkle chain API**: the original sonam-plugin-rnd commit imported `compute_record_hash` / `previous_record_hash` / `sign_hash` which no longer exist — now uses `ChainValues.sequence_number`/`previous_record_id` + `sign_canonical` to match orchestrator's `_apply_session_chain`. Any future external contribution that touches session chain must reference `src/gateway/pipeline/orchestrator.py:_apply_session_chain` as the source of truth.
- **model_hash plumbing (end-to-end)**: `CachedAttestation.model_hash` field, `attestations.model_hash` SQLite column with idempotent `ALTER TABLE ADD COLUMN model_hash TEXT DEFAULT ''` migration (try/except-wrapped for existing DBs), `ControlPlaneStore.update_model_hash(provider, model_id, tenant_id, model_hash)` backfill method (only writes if empty — never overwrites), `_discover_ollama` returns each model's `digest` alongside `model_id`/`provider`/`source`, both `_after_stream_record` and `_maybe_fetch_ollama_hash` in orchestrator call `ctx.control_store.update_model_hash(...)` after `adapter.fetch_model_hash(...)` succeeds so attestations get hashed on first real use without a separate workflow.
- **Dashboard — Walacor wordmark**: new `components/WalacorWordmark.jsx` renders theme-aware PNG (dark/light branding asset). `SealButton` replaces literal "◆ SEALED IN WALACOR" text with `<span>SEALED IN</span> <WalacorWordmark size="seal" />`; `SealDrawer` eyebrow does the same with `size="eyebrow"`. Assets live at `src/gateway/lineage/dashboard/src/assets/branding/walacor-logo-{dark,light}.png`. CSS handles `filter: brightness(0)` silhouette on gold hover.
- **`.gitignore` *.png gotcha**: root `.gitignore` had blanket `*.png` from the screenshots rule. Silently excluded the wordmark PNGs — build failed on every fresh checkout with `Could not resolve "../assets/branding/walacor-logo-dark.png"`. Fixed with explicit exception `!src/gateway/lineage/dashboard/src/assets/**/*.png`. Any future dashboard asset (icons, logos, illustrations) MUST live under `dashboard/src/assets/` to survive the root ignore.
- **Sessions list fetch-error banners** (`views/Sessions.jsx`): distinguishes three no-data states — `fetchError.kind === 'auth'` → amber banner with "Go to Control" button calling `navigate('control')`; `fetchError.kind === 'error'` → red banner with `fetchError.detail` (copy-paste friendly); empty with no error → friendlier "No sessions yet — send traffic through the gateway" copy. Styles in `sessions-v2.css`: `.ses-fetch-banner`, `.ses-fetch-banner--auth`, `.ses-fetch-banner--error`.
- **UnlockModal hardening** (`Control.jsx:UnlockModal`): added `role="dialog"`, `aria-modal="true"`, `aria-labelledby`; mousedown/click guards on the wrap `onMouseDown={e => { if (e.target === e.currentTarget) e.preventDefault(); }}` so accidental drag-outside-text doesn't dismiss; `useEffect` body-overflow lock while open. Pair these patterns on any future modal.
- **Connections page H1**: renamed from "Is anything silently broken right now?" to "Subsystem health check". The eyebrow "CONNECTIONS · SILENT-FAILURE SURFACE" already carries the silent-failure framing; the H1 should name what you're looking at, not restate it.
- **Control + Connections bordered hero**: `cp-intro` and `cx-intro` now use `card card-accent` classes to match Compliance's bordered hero treatment (instead of the old flat section divider with `border-bottom`). Margin/padding consolidated: `margin-bottom: 18px`, `border-bottom: none`, `.cp-intro.card { padding: 28px 32px; }`.
