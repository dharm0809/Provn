...
- **Walacor sandbox never anchors**: `BlockId/TransId/DH` stay null on sandbox.walacor.com across all ETIds. INT-04 is therefore demoted to `severity=warn` so a permanently un-anchored sandbox stays `degraded`, not `unready`.
