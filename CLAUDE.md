...
`WALACOR_OBSERVABILITY_AUTH_REQUIRED` (default `false`) gates the operational endpoints that leak capacity/state: when `true`, `/v1/readiness`, `/v1/connections`, and `/metrics` require a valid `X-API-Key` from `api_keys_list` (401 on missing, 403 on wrong key — distinct from the standard middleware's flat 401). `/health` stays public so load balancers can probe, but the response body strips debug fields (`storage`, `attestation_cache`, `policy_cache`, `wal`, `token_budget`, `intelligence`, `resource_monitor`, `model_capabilities`, `startup_probes`, `content_analyzers`, `session_chain`) unless the caller presents a valid key. The gating lives in the existing `api_key_middleware` (do not add a second auth middleware); the strip happens in `health_response` itself.
