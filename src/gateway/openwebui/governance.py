...
async def _apply_session_chain(record: dict, session_id: str | None, ctx, settings) -> str | None:
    """Compute and attach session chain fields.  Returns record_hash or None."""
    ...
    # Ed25519 record signing (fail-open)
    try:
    ...
    try:
        await ctx.session_chain.update(session_id, seq_num, record_hash_val)
    except Exception:
        logger.error(
            "Plugin event session chain update failed: session_id=%s seq=%d",
